...
### 1. 메타데이터 기반 동적 페이지 렌더링

일부 복잡한 화면을 제외한 페이지는 화면 구성을 프론트엔드에 직접 하드코딩하지 않고,
백엔드에서 전달하는 메타데이터를 기반으로 동적으로 렌더링하도록 설계하고 있습니다.

백엔드는 페이지 컨텍스트에 따라 Top Navigation, Section 정보, 권한 정보를 조합하여 제공하며,
프론트엔드는 해당 메타데이터를 해석하여 화면을 구성합니다.
